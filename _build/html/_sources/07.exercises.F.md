...
To rationalize the data the econometrician assumes that the utility of each alternative is given by a scalar product of a vector of parameters $\beta \in \mathbb{R}^2$ and a vector of alternative characteristics $x_j \in \mathbb{R}^2$, $j=1,2$.
...
In order to estimate the vector of parameters of the model $\beta$, the econometrician maximizes the likelihood of observing the data $D = (x,y) = \big(\{(x_{1j},x_{2j})\}_{j \in \{1,2\}},\{y_i\}_{i \in \{1,\dots,N\}}\big)$. The likelihood function $L \colon \mathbb{R}^{2+2J+N} \to \mathbb{R}$ is given by
%
$$
L(\beta,D) = \prod_{i=1}^N \ell_i(\beta,x,y_i),
$$
where the individual likelihood contribution of data point $i$ is given by a function $\ell_i \colon \mathbb{R}^7 \to \mathbb{R}$
%
$$
\ell_i(\beta,x,y_i) = 
\left[p_1 \left(
