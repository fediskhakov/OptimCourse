...
If $\lambda = 0$, then $x=y=0$, which contradicts with $x^2 + y^2 - 2a^2 = 0$ and $a>0$. Thus, $\lambda \neq 0$.

If $x=0$, then $\lambda = y = 0$, which contradicts with $\lambda \neq 0$. Thus, $x \neq 0$.

Similarly, we can prove $y \neq 0$.

Thus, from $y = 2 \lambda x, x = 2 \lambda y$ and $x, y, \lambda \neq 0$ we know $x=y$ or $x=-y$. Plug this into 
$x^2 + y^2 = 2a^2$ we get four critical points:
