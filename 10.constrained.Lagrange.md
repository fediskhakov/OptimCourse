...
> Importantly, the same derivation holds for each leading principle minor of $E$!

Therefore, the definiteness of the quadratic form given by $E$ can be checked by examining the determinants of the matrixes obtained form the bordered $H$ by removing last $j$ columns and bottom $j$ rows, for $j=1,\dots,N-K$.

Let's compute the order-$j$ leading principle minor of $E$ (quadratic form subject to the linear constraint) through the elements of the bordered matrix $H$. 
...
the last $N-K$ leading principal minors of bordered Hessian $H \mathcal{L}(\lambda,x)$ are zero or alternate in sign, with the sign of the determinant of the full Hessian matrix equal to $(-1)^N$

- If $f$ has a **local minimum** on $D$ at $x^\star$, then the last $N-K$ leading principal minors of bordered Hessian $H \mathcal{L}(\lambda,x)$ are zero or have the same sign as $(-1)^K$
