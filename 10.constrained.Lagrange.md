...
Here the newly denoted matrix $J$ is $(K \times N-K)$, and we can write the full vector $x: Bx=0$ in terms of parameters $(x_{K+1},\dots,x_{N})$ using matrix $J$ and the $(N-K \times N-K)$ identity matrix $I_{N-K}$ as
...
Now we can study the definiteness of quadratic form $E$ instead of definiteness of the original quadratic form subject to the linear constraint!

To do it conveniently, consider a $(N+K \times N+K)$ bordered matrix 
...
With an additional (change of basis) matrix 
...
J^TB_1^T+B_2^T & J^TA_{11}+A_{21} & J^TA_{11}J+A_{21}J+J^T A_{12}J+A_{22}
\end{pmatrix}
=
$$

Note that $B_1 J + B_2 = B_1 (-B_1^{-1}B_2) + B_2 = 0$, and verify that the lower right block is just $E$!
Denote $C=A_{11}J+A_{12}$, and then $C^{T}=J^TA_{11}^T+A_{12}^T=J^TA_{11}+A_{21}$ since $A$ is symmetric.
...
Next, let's compute the determinant of bordered matrix $H$. First, note that
...
Clearly, the determinant of the change of basis matrix is 1, and we have
...
Dealing with determinants of the block matrixes, the useful formula is
...
> Importantly, the same derivation holds for each leading primary minor of $E$!

Therefore, the definiteness of the quadratic form given by $E$ can be checked by examining the determinants of the form

Let's compute the order-$j$ leading primary minor of $E$ (quadratic form subject to the linear constraint) through the elements of the bordered matrix $H$. 
...
- similarly, $B^j_2$ is the first $j$ columns of the matrix $B_2$, $(K \times j)$ matrix

> In other words, the oder-$j$ leading principle minor of $E$ is given by the determinant of the bordered matrix $H$ where the last $$ rows and columns are dropped!

````{admonition} Definiteness of quadratic form subject to linear constraint
:class: important

Hence, in order to establish definiteness of the quadratic form $x^TAx$, $x \in \mathbb{R}^N$, subject to a linear constraint $Bx=0$, we need to check signs of the $N-K$ leading principle minors of the $(N-K \times N-K)$ matrix $E$ which represents the equivalent lower dimension quadratic form without the constraint.
...
We can now reformulate the second order conditions for the equality constrained optimization problems in terms of signs of the *last* leading principle minors of the bordered Hessian of the Lagrangian function.
...
- If $f$ has a **local minimum** on $D$ at $x^\star$, then the last $N-K$ leading principal minors of $H \mathcal{L}(\lambda,x)$ are zero or have the same sign as $(-1)^K$
...
- If the last $N-K$ leading principal minors of $H_x \mathcal{L}(\lambda,x)$ defined above are non-zero and alternate in sign, with the sign of the full Hessian matrix equal to $(-1)^N$,
then $x^\star$ is the **local constrained maximum** of $f$ on $D$

- If the last $N-K$ leading principal minors of $H \mathcal{L}(\lambda,x)$ defined above are non-zero and have the same sign as $(-1)^K$,
then $x^\star$ is the **local constrained minimum** of $f$ on $D$
